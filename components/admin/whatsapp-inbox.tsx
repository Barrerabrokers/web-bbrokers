...
import { Bot, Check, CircleUserRound, Loader2, LockKeyhole, MessageCircle, Send, UserPlus } from "lucide-react";
import type { WhatsAppConversation, WhatsAppMessage } from "@/lib/whatsapp-inbox";

type AgentOption = { id: string; name: string };

export function WhatsAppInbox({ initialConversations, agents, isAdmin, configured }: { initialConversations: WhatsAppConversation[]; agents: AgentOption[]; isAdmin: boolean; configured: boolean }) {
  ...
  return (
    <main className="bg-cream-100 px-4 py-6 sm:px-6">
      <div className="mx-auto max-w-[1600px]">
        <div className="mb-4 flex flex-col justify-between gap-3 rounded-md border border-ink/12 bg-white p-5 sm:flex-row sm:items-center">
          <div><p className="text-xs font-semibold uppercase tracking-[.22em] text-ink/45">WhatsApp oficial</p><h2 className="mt-1 text-2xl font-semibold text-ink">Bandeja compartida</h2><p className="mt-1 text-sm text-ink/55">Un número, responsables claros y toda la conversación guardada en el CRM.</p></div>
          <span className={`inline-flex items-center gap-2 rounded-full px-3 py-2 text-xs font-semibold ${configured ? "bg-emerald-50 text-emerald-700" : "bg-amber-50 text-amber-800"}`}>{configured ? <Check className="h-4 w-4" /> : <LockKeyhole className="h-4 w-4" />}{configured ? "API conectada" : "Pendiente de credenciales Meta/OpenAI"}</span>
        </div>
        <div className="grid min-h-[680px] overflow-hidden rounded-md border border-ink/12 bg-white lg:grid-cols-[360px_1fr]">
          <aside className="border-b border-ink/10 lg:border-b-0 lg:border-r">
            <div className="border-b border-ink/10 p-4"><strong className="text-sm text-ink">Conversaciones</strong><p className="text-xs text-ink/48">{conversations.length} chats visibles</p></div>
            <div className="max-h-[620px] overflow-y-auto">
              {!conversations.length && <div className="p-8 text-center text-sm text-ink/50"><MessageCircle className="mx-auto mb-3 h-8 w-8 opacity-40" />Todavía no hay conversaciones asignadas.</div>}
              {conversations.map((conversation) => <button key={conversation.id} onClick={() => setSelectedId(conversation.id)} className={`w-full border-b border-ink/8 p-4 text-left transition ${selectedId === conversation.id ? "bg-cream-100" : "hover:bg-cream-50"}`}><span className="flex items-start justify-between gap-3"><strong className="truncate text-sm text-ink">{conversation.contactName || conversation.phone}</strong><small className="shrink-0 text-[10px] text-ink/42">{new Date(conversation.lastMessageAt).toLocaleTimeString("es-AR", { hour: "2-digit", minute: "2-digit" })}</small></span><span className="mt-1 block truncate text-xs text-ink/52">{conversation.lastMessage}</span><span className="mt-2 flex items-center gap-2 text-[11px] text-ink/48">{conversation.aiEnabled ? <><Bot className="h-3.5 w-3.5 text-accent" /> IA activa</> : <><CircleUserRound className="h-3.5 w-3.5" />{conversation.assignedAgentName || "Sin propietario"}</>}{conversation.unreadCount > 0 && <b className="ml-auto rounded-full bg-accent px-2 py-0.5 text-white">{conversation.unreadCount}</b>}</span></button>)}
            </div>
          </aside>
          <section className="flex min-w-0 flex-col">
            {!selected ? <div className="grid flex-1 place-items-center p-8 text-sm text-ink/50">Seleccioná una conversación.</div> : <>
              ...
              <form onSubmit={send} className="flex gap-3 border-t border-ink/10 p-4"><input value={draft} onChange={(event) => setDraft(event.target.value)} placeholder={configured ? "Escribí un mensaje…" : "Configurá la API para enviar mensajes"} disabled={!configured || loading} className="min-w-0 flex-1 rounded-md border border-ink/15 px-4 py-3 text-sm outline-none focus:border-accent" /><button disabled={!configured || loading || !draft.trim()} className="inline-flex items-center gap-2 rounded-md bg-accent px-4 py-3 text-sm font-semibold text-white disabled:opacity-40">{loading ? <Loader2 className="h-4 w-4 animate-spin" /> : <Send className="h-4 w-4" />}Enviar</button></form>
            </>}
          </section>
        </div>
      </div>
