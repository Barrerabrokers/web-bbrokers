              ...
              <div className="mt-2 rounded-xl bg-[#f3f4f4] p-3 ring-1 ring-ink/10">
                <div className="mx-auto max-w-2xl overflow-hidden rounded-lg bg-white px-4 py-5 sm:px-6">
                  {contentBlocks.map((block) => {
                    ...
                    if (block.type === "columns") return <div key={block.id} className="my-3 grid items-stretch" style={{ gap: `${block.gap ?? 12}px`, gridTemplateColumns: (block.widths || block.columns.map(() => 1)).map((width) => `${width}fr`).join(" ") }}>{block.columns.map((column, index) => column.type === "text" ? <div key={index} className="max-w-full overflow-hidden whitespace-pre-wrap break-words leading-relaxed" style={{ color: column.color || "#1f2933", fontSize: `${column.fontSize || 14}px`, fontFamily: column.fontFamily, textAlign: column.align || "left", fontWeight: column.bold ? 700 : 400, overflowWrap: "anywhere" }} dangerouslySetInnerHTML={{ __html: column.html || column.text }} /> : <div key={index} className="relative h-full min-h-24 overflow-hidden" style={{ borderRadius: `${column.borderRadius ?? 8}px` }}><img src={column.url} alt={column.alt || "Imagen de la plantilla"} className="absolute inset-0 h-full w-full object-cover" /></div>)}</div>;
