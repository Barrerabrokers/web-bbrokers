...
import { AlignCenter, AlignLeft, AlignRight, Bold, Braces, Highlighter, ImagePlus, Italic, Link, List, ListOrdered, Loader2, Palette, Paperclip, Redo2, RemoveFormatting, Trash2, Underline, Undo2 } from "lucide-react";
...
export function CrmRichEmailEditor({ blocks, onChange, onNotice, variables = [], onUploadingChange }: { onUploadingChange?: (value: boolean) => void; blocks: Blocks; onChange: (blocks: Blocks) => void; onNotice: (message: string) => void; variables?: EditorVariable[] }) {
  ...
  const attachmentInput = useRef<HTMLInputElement>(null); const latestBlocks = useRef(blocks); latestBlocks.current = blocks;
  ...
  const uploadAttachment = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (files.some(file => file.size > 10 * 1024 * 1024)) return onNotice("Cada adjunto puede pesar hasta 10 MB. Seleccioná una versión más liviana.");
    setUploading(true);
    try {
      for (const file of files) {
        const url = await uploadCrmEmailAttachment(file);
        const next: Blocks = [...latestBlocks.current, { id: crypto.randomUUID(), type: "attachment", url, name: file.name }];
        latestBlocks.current = next; onChange(next);
      }
      onNotice("Archivos adjuntados. Se enviarán junto con el correo y se conservarán si guardás la plantilla.");
    } catch (error) { onNotice(error instanceof Error ? error.message : "No se pudieron adjuntar los archivos."); }
    finally { setUploading(false); }
  };
  const upload = async (event: ChangeEvent<HTMLInputElement>) => { const file = event.target.files?.[0]; event.target.value = ""; if (!file) return; setUploading(true); try { const form = new FormData(); form.append("files", file); form.append("folder", "templates"); const response = await fetch("/api/upload", { method: "POST", body: form }); const data = await response.json().catch(() => null) as { urls?: string[]; error?: string } | null; if (!response.ok || !data?.urls?.[0]) throw new Error(data?.error || "No se pudo cargar la imagen."); const image: CrmEmailTemplateContentBlock = { id: crypto.randomUUID(), type: "image", url: data.urls[0], alt: file.name, width: 100, align: "center", borderRadius: 8 }; const index = blocks.findIndex((block) => block.id === activeKey.split("::")[0]); const next = [...blocks]; next.splice(index < 0 ? next.length : index + 1, 0, image); onChange(next); onNotice("Imagen agregada en el lugar elegido."); } catch (error) { onNotice(error instanceof Error ? error.message : "No se pudo cargar la imagen."); } finally { setUploading(false); } };
  ...
  return <section className="mt-2 overflow-visible rounded-xl border border-ink/15 bg-[#f3f4f4]">
    <div className="sticky top-0 z-20 rounded-t-xl border-b border-ink/10 bg-white px-3 py-2 shadow-[0_2px_6px_rgba(0,0,0,0.06)]"><div className="flex flex-wrap items-center gap-1.5">
      ...
      <button type="button" title="Hasta 10 MB por archivo" onClick={() => attachmentInput.current?.click()} disabled={uploading} className="inline-flex min-h-9 items-center gap-2 rounded-lg border border-[#006b6b] px-3 text-xs font-semibold text-[#006b6b] disabled:opacity-50"><Paperclip className="h-4 w-4" />{uploading ? "Subiendo…" : "Adjuntar archivo"}</button><input ref={attachmentInput} type="file" multiple accept=".pdf,.doc,.docx,.xls,.xlsx,.csv,.txt,.ppt,.pptx,.jpg,.jpeg,.png,.webp,.gif" aria-label="Seleccionar archivos adjuntos" className="hidden" onChange={uploadAttachment} />
    ...
    <div className="max-h-[52vh] overflow-y-auto overscroll-contain px-2 py-4 sm:px-5"><div className="mx-auto w-full max-w-[640px] overflow-hidden rounded-lg bg-white px-4 py-5 shadow-sm sm:px-7">{blocks.map((block) => {
      ...
      if (block.type === "divider") return <hr key={block.id} className="my-4 border-0" style={{ height: `${block.thickness || 1}px`, width: `${block.width || 100}%`, backgroundColor: block.color || "#d8dddf" }} />; if (block.type === "spacer") return <div key={block.id} style={{ height: `${block.height}px` }} />; return <div key={block.id} className="my-3 flex items-center gap-3 rounded-lg border border-ink/15 bg-[#f3f4f4] px-3 py-3 text-sm"><Paperclip className="h-4 w-4 shrink-0 text-[#006b6b]" /><div className="min-w-0 flex-1"><a href={block.url} target="_blank" rel="noopener noreferrer" className="break-all font-medium text-[#006b6b] underline">{block.name}</a><p className="text-xs text-ink/65">Se enviará como archivo adjunto</p></div><button type="button" aria-label={`Quitar adjunto ${block.name}`} disabled={uploading} onClick={() => onChange(blocks.filter(item => item.id !== block.id))} className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg text-red-700 hover:bg-red-50"><Trash2 className="h-4 w-4" /></button></div>;
    })}</div></div>
  </section>;
}
