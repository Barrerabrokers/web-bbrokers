...
export function CrmRichEmailEditor({ blocks, onChange, onNotice }: { blocks: Blocks; onChange: (blocks: Blocks) => void; onNotice: (message: string) => void }) {
  ...
  return <section className="mt-2 overflow-hidden rounded-xl border border-ink/15 bg-[#f3f4f4]">
    ...
    </div><p className="mt-1.5 text-[11px] text-ink/50">Seleccioná texto y aplicá el formato. Lo que ves es lo que recibirá el cliente.</p></div>
    <div className="max-h-[56vh] overflow-y-auto px-2 py-4 sm:px-5"><div className="mx-auto w-full max-w-[640px] overflow-hidden rounded-lg bg-white px-4 py-5 shadow-sm sm:px-7">{blocks.map((block) => {
