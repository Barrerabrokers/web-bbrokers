...
export function CrmContactAssistant({ canViewAll, contactId, initialQuestion = "", buttonLabel = "Consultar IA" }: { canViewAll: boolean; contactId?: string; initialQuestion?: string; buttonLabel?: string }) {
  ...
  async function ask(event: FormEvent) {
    ...
    try {
      const response = await fetch("/api/crm/contact-assistant", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ question: text, conversationId: conversationId || undefined, contactId }) });
      const data = await readResponse(response);
  ...
  const openAssistant = () => {
    if (!messages.length && !question.trim() && initialQuestion) setQuestion(initialQuestion);
    setOpen(true);
  };

  return <>
    <button type="button" onClick={openAssistant} className="fixed bottom-24 right-4 z-40 inline-flex min-h-12 items-center gap-2 rounded-full bg-[#005c5c] px-4 text-sm font-semibold text-white shadow-[0_6px_8px_rgba(0,92,92,0.22)] transition-transform hover:-translate-y-0.5 hover:bg-[#004949] lg:bottom-6 lg:right-7"><Sparkles className="h-4 w-4" />{buttonLabel}</button>
