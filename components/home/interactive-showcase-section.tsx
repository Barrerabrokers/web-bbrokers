...
export function InteractiveShowcaseSection({
  ...
  if (items.length === 0) {
    return (
      <section className="min-h-[60vh] flex items-center justify-center" style={{ backgroundColor: "var(--oa-bg-cream)" }}>
  ...
  // Colores warm
  const BG       = "var(--oa-bg-cream)";
  const TEXT     = "var(--oa-black)";
  const MUTED    = "rgba(7,7,7,0.5)";
  const BORDER   = "rgba(7,7,7,0.1)";
  const OVERLAY  = "rgba(58,29,23,0.75)";
  const BADGE_BG = "rgba(239,230,216,0.88)";
  const BADGE_FG = "var(--oa-brown)";
  const NAV_BG   = "rgba(248,245,239,0.75)";
  ...
  return (
    <section
      ...
      style={{
        background: `linear-gradient(180deg, rgba(184,157,135,0.12) 0%, ${BG} 12%, #e8ddd0 88%, ${BG} 100%)`,
      }}
    >
      {/* Líneas diagonales */}
      ...
      {/* Grain */}
      <div aria-hidden="true" className="absolute inset-0 pointer-events-none bg-grain opacity-40" />
      ...
      <motion.div style={{ scale: parallaxScale, opacity: parallaxOpacity }} className="relative z-10 container-custom py-24 md:py-32 lg:py-40">
        ...
        <div className="mb-16 md:mb-24">
          ...
          <motion.h2
            ...
            style={{ fontSize: "clamp(2.2rem, 5vw, 5rem)", color: TEXT }}
          >
            {heading}
          </motion.h2>
          <motion.p
        ...
        <div className="relative grid grid-cols-12 gap-4 md:gap-6 min-h-[60vh] items-center">
          ...
          {secondaryItems[0] && (
            <motion.div style={{ y: parallaxY1 }} className="hidden lg:block col-span-3 col-start-1">
              <SideCard item={secondaryItems[0]} delay={0.3} hasEntered={hasEntered} />
            </motion.div>
          )}
                ...
                <AnimatePresence initial={false} custom={direction} mode="wait">
                  ...
                  >
                    <Link href={active.href} className="block h-full group">
                      ...
                      {/* Warm overlay */}
                      <div className="absolute inset-0" style={{ background: `linear-gradient(to top, ${OVERLAY} 0%, rgba(58,29,23,0.3) 45%, transparent 100%)` }} />
                      ...
                      {active.statusLabel && (
                        <div className="absolute top-5 left-5">
                          <span className="px-4 py-1.5 text-[9px] uppercase tracking-[0.2em] font-medium rounded-full" style={{ background: BADGE_BG, backdropFilter: "blur(16px)", color: BADGE_FG, border: "1px solid rgba(58,29,23,0.15)", fontFamily: "var(--font-sans)" }}>
                            {active.statusLabel}
                          </span>
                        </div>
                      )}
                        ...
                        <AnimatePresence mode="wait" custom={direction}>
                          ...
                          >
                            <div className="flex items-center gap-2 mb-3" style={{ color: "rgba(248,245,239,0.7)" }}>
                              ...
                              <span className="text-[10px] uppercase tracking-[0.18em]" style={{ fontFamily: "var(--font-sans)" }}>{active.location}</span>
                            </div>
                            ...
                            <div className="flex items-center gap-6 flex-wrap">
                              {active.priceLabel && (
                                <div>
                                  ...
                                  <span className="font-display text-lg" style={{ color: "var(--oa-bg-light)" }}>{active.priceLabel}</span>
                                </div>
                              )}
                              {active.completionDate && (
                                <div>
                                  ...
                                  <span className="font-display text-lg" style={{ color: "var(--oa-white)" }}>{active.completionDate}</span>
                                </div>
                              )}
                              {active.extraStats?.map((stat) => (
                                <div key={stat.label}>
                                  <p className="text-[8px] uppercase tracking-[0.2em] mb-0.5" style={{ color: "rgba(248,245,239,0.5)", fontFamily: "var(--font-sans)" }}>{stat.label}</p>
                                  <span className="font-display text-lg" style={{ color: "var(--oa-white)" }}>{stat.value}</span>
                                </div>
                              ))}
                            </div>
                          </motion.div>
                      ...
                      <div className="absolute bottom-6 right-6 md:bottom-8 md:right-8">
                        <div
                          className="h-11 w-11 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 translate-y-2 group-hover:translate-y-0 transition-all duration-500"
                          style={{ background: "var(--oa-white)", border: "1px solid rgba(7,7,7,0.1)" }}
                        >
                ...
                <div className="absolute top-1/2 -translate-y-1/2 left-0 right-0 z-30 flex justify-between px-4 md:-mx-8 pointer-events-none">
                  {[{ fn: prevProject, Icon: ArrowLeft, label: "Anterior" }, { fn: nextProject, Icon: ArrowRight, label: "Siguiente" }].map(({ fn, Icon, label }) => (
                    <button
                      key={label}
                      onClick={fn}
                      className="pointer-events-auto h-13 w-13 rounded-full flex items-center justify-center transition-all duration-300 hover:scale-110"
                      style={{ width: "52px", height: "52px", background: NAV_BG, backdropFilter: "blur(16px)", border: "1px solid rgba(7,7,7,0.1)" }}
                      aria-label={label}
                    >
          ...
          {secondaryItems[1] && (
            <motion.div style={{ y: parallaxY2 }} className="hidden lg:block col-span-3 col-start-10">
              <SideCard item={secondaryItems[1]} delay={0.5} hasEntered={hasEntered} />
            </motion.div>
          )}
        </div>
          ...
          <motion.div style={{ y: parallaxY3 }} className="mt-16 md:mt-24 grid grid-cols-2 md:grid-cols-4 gap-4">
            {secondaryItems.slice(0, 4).map((item, idx) => (
              ...
              >
                ...
                >
                  {item.image && (
                    <Image src={item.image} alt={item.title} fill className="object-cover transition-transform duration-[2000ms] ease-out group-hover:scale-[1.05]" sizes="(max-width: 768px) 50vw, 25vw" />
                  )}
                  <div className="absolute inset-0" style={{ background: "linear-gradient(to top, rgba(58,29,23,0.8) 0%, rgba(58,29,23,0.2) 50%, transparent 100%)" }} />
                  ...
                  <div className="absolute bottom-3 left-3 right-3">
                    <p className="text-[8px] uppercase tracking-[0.18em] mb-0.5" style={{ color: "rgba(248,245,239,0.55)", fontFamily: "var(--font-sans)" }}>{item.location}</p>
                    <h4 className="font-display font-light text-sm leading-tight truncate group-hover:italic transition-all duration-500" style={{ color: "var(--oa-white)" }}>{item.title}</h4>
                    {item.priceLabel && (
                      <p className="text-[10px] mt-1 font-display" style={{ color: "var(--oa-bg-light)" }}>{item.priceLabel}</p>
                    )}
                  </div>
                </Link>
              </motion.div>
...
/* ── Side card ── */
function SideCard({ item, delay, hasEntered }: { item: ShowcaseItem; delay: number; hasEntered: boolean }) {
  return (
    <motion.div
    ...
    >
      ...
      >
        {item.image && (
          <Image src={item.image} alt={item.title} fill className="object-cover transition-transform duration-[2500ms] ease-out group-hover:scale-[1.06]" sizes="25vw" />
        )}
        ...
        <div className="absolute bottom-4 left-4 right-4">
          <p className="text-[8px] uppercase tracking-[0.18em] mb-1" style={{ color: "rgba(248,245,239,0.55)", fontFamily: "var(--font-sans)" }}>{item.location}</p>
          <h4 className="font-display font-light text-base leading-tight group-hover:italic transition-all duration-500" style={{ color: "var(--oa-white)" }}>{item.title}</h4>
        </div>
      </Link>
    </motion.div>
  );
