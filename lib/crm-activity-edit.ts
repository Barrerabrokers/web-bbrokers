...
function version(row: {title:string;body:string|null;scheduled_at:unknown}, outcome:string) {
  return createHash("sha256").update(JSON.stringify([row.title,row.body || "",row.scheduled_at ? new Date(String(row.scheduled_at)).toISOString() : "",outcome])).digest("hex");
}
async function details(sql: Sql, ids: string[]) {
  const rows = await sql`SELECT a.id,a.title,a.body,a.scheduled_at,r.outcome,r.updated_at,agent.name AS updated_by_name
  ...
  return rows.map(r=>({id:r.id as string, editVersion:version(r as any,r.outcome || ""),meetingOutcome:(r.outcome || "") as string,
...
export async function editCrmActivity(input:{id:string;version:string;title?:string;body?:string;scheduledAt?:string;outcome?:string},actor:{id:string;includeAll:boolean}) {
