...
export function normalizeDialCode(value?: string | null) {
  ...
  if (!cleaned) return "+54";
  const digits = cleaned.replace(/\D/g, "");
  if (!digits) return "+54";
  return `+${digits}`;
}

export function dialCodeForIso2(value?: string | null) {
  const iso2 = String(value || "").trim().toUpperCase();
  return PHONE_COUNTRIES.find((country) => country.iso2 === iso2)?.dialCode;
}

export function splitInternationalPhone(rawPhone?: string | null, fallbackCountry?: string | null) {
  const fallbackDialCode = dialCodeForIso2(fallbackCountry) || normalizeDialCode(fallbackCountry) || "+54";
  const cleaned = String(rawPhone || "").trim();
  if (!cleaned) return { countryCode: fallbackDialCode, phone: "" };

  const digitsOnly = cleaned.replace(/\D/g, "");
  const plusDigits = cleaned.startsWith("+") ? `+${digitsOnly}` : "";
  const candidates = PHONE_COUNTRIES
    .map((country) => country.dialCode)
    .filter((dialCode, index, list) => list.indexOf(dialCode) === index)
    .sort((a, b) => b.length - a.length);

  const international = plusDigits || (digitsOnly.startsWith("00") ? `+${digitsOnly.slice(2)}` : "");
  if (international) {
    const matchedDialCode = candidates.find((dialCode) => international.startsWith(dialCode));
    if (matchedDialCode) {
      const dialDigits = matchedDialCode.replace(/\D/g, "");
      let subscriberNumber = international.slice(matchedDialCode.length).replace(/\D/g, "");
      while (
        dialDigits &&
        subscriberNumber.startsWith(dialDigits) &&
        subscriberNumber.length > dialDigits.length + 4
      ) {
        subscriberNumber = subscriberNumber.slice(dialDigits.length);
      }
      return {
  ...
  if (fallbackDigits && digitsOnly.startsWith(fallbackDigits) && digitsOnly.length > fallbackDigits.length + 4) {
    let subscriberNumber = digitsOnly;
    while (
      subscriberNumber.startsWith(fallbackDigits) &&
      subscriberNumber.length > fallbackDigits.length + 4
    ) {
      subscriberNumber = subscriberNumber.slice(fallbackDigits.length);
    }
    return {
